...
Choose a page count that matches the available evidence and carry the reader from recognition to strategic conviction, then from conviction to a credible first proof.

## Table of contents

1. Invariant argument
2. Profile selection
...
The reusable structure is rhetorical, not visual:

```text
enter the brand world
→ understand the reading path
→ recognize the current moment
→ respect what already works
→ expose the gap or risk
→ reveal the opportunity
→ name a proprietary platform
→ prove the platform can operate
→ prioritize a first move
→ show the proposed delivery can be executed
→ close with restraint
```

Every page must advance this chain. If removing a page changes nothing, remove or merge it.
...
This profile follows the compact factory logic and derives its art direction from the current prospect.
...
Write `strategy/page-map.md` before full copy:

```markdown
## NN - [Page role]

- Family: [typed engine family and variant]
- Act: [I, II, III or IV]
- Purpose: [one rhetorical job]
- Takeaway: [one evidence-backed sentence written for this prospect]
- Claims: [C01, C02]
- Claim paths: [title, columns[0].body]
- Evidence: [E01, O01]
- Blocks:
  1. [block role]
  2. [block role]
...
- Copy risk: [specific factual or interpretive risk]
```
...
- `meta.stage` is `final` before the final validation, render, audit and delivery.
...
Use chapter openers to:

- introduce a new question;
- switch from diagnosis to platform;
- switch from platform to routes;
- switch from routes to chosen activation;
- conclude.

Mark the change through background, motif state, image role, chapter marker or whitespace. Do not introduce a completely new art direction.

## 8. Compression and expansion

### Compress when

- two pages make the same claim;
- the brand has one product and routes would be artificial;
- references are unverified;
- no detailed execution proof exists;
- the first outreach benefits from a faster read.

Useful merges:

- brand truth + existing foundation;
- perception gap + risk;
- territory + platform;
...
- distinct product models need distinct tensions;
- the platform has several audiences or channels with real differences;
- a storyboard, prototype or shot list exists;
- the company has important proof assets that deserve legibility;
- the user supplied specific film or campaign concepts.

Useful splits:

- proof into current communication and operational capability;
- platform into manifesto and operating system;
- routes into one page per product or audience;
- execution into concept, storyboard and rollout;
- references and thank-you into separate pages.

Never expand a page by repeating the same sentence in a new visual arrangement.
