...
import { analyzeImage, type ImageInventoryEntry } from "./image-analysis.js";
...
function makeMarkdown(manifest: SourceAuditManifest): string {
...
export async function runSourceAudit(options: SourceAuditOptions): Promise<SourceAuditManifest> {
...
export async function sourceAuditCli(arguments_: readonly string[]): Promise<number> {
  try {
