...
function key(role: unknown, family: unknown, style: unknown, weight: unknown): string {
...
function inspectContract(value: readonly unknown[], issues: string[]): Map<string, ExpectedFace> {
  const expected = new Map<string, ExpectedFace>();
  value.forEach((entry, index) => {
    ...
    entry.weights.forEach((weight, weightIndex) => {
      ...
      if (expected.has(faceKey)) issues.push(`Render report font contract contains duplicate face: ${faceKey}.`);
...
export function validateRenderFonts(value: unknown, issues: string[]): void {
  ...
  value.faces.forEach((entry, index) => {
    ...
    if (actual.has(faceKey)) issues.push(`Render report font audit contains duplicate face: ${faceKey}.`);
    actual.add(faceKey);
