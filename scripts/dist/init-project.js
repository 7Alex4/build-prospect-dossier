...
Options:
  ...
  --help                               Show this help
`;
export function defaultTemplateDirectory(importMetaUrl = import.meta.url) {
    return path.resolve(path.dirname(fileURLToPath(importMetaUrl)), "../../assets/dossier-engine");
}
export async function initProjectCli(arguments_) {
    try {
        ...
        if ((authorizationReference === undefined) !== (authorizedBy === undefined)) {
            throw new UserInputError("--authorize-generative-assets and --generative-assets-authorized-by must be provided together.");
        }
