import { validateRenderAssets, } from "./render-report-asset-validation.js";
...
function fileIntegrity(value, label, issues) {
...
function validateEvidence(value, issues) {
    ...
    value.forEach((entry, index) => {
        ...
        if (ids.has(entry.id))
            issues.push(`Render report contains duplicate evidence ID: ${entry.id}.`);
        ids.add(entry.id);
...
function referencedClaimEvidence(claims, issues, index) {
...
function validateTraceability(value, renderedSlideIds, assetIds, evidenceIds, themeAssetIds, issues) {
    if (!Array.isArray(value)) {
...
function validateIntegrity(value, pages, slideIds, issues) {
    if (!isRecord(value)) {
...
export function inspectRenderReportStructure(parsed, orderedPages) {
    ...
    if (value.overflow !== 0)
        issues.push("Render report overflow must be zero.");
    ...
    if (themeAssetIds === null)
        issues.push("Render report themeAssetIds must be an array.");
...
//# sourceMappingURL=render-report-structure.js.map